...
export const Question = memo(
    ({ question, setQuestionAnswer, isAnswerDone, setUserAnswer, userAnswers }: QuestionProps) => {
        const answerClickHandler = (event: FormEvent<HTMLFormElement>) => {
            event.preventDefault()
            setQuestionAnswer(
                question.options.findIndex((el) =>
                    el.isRight ? !userAnswers.includes(el.id) : userAnswers.includes(el.id),
                ) === -1,
            )
        }
